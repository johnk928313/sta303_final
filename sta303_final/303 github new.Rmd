...
# read the data
library(parameters)
black_saber_current_employees <- current_employees
...
phase3 %>%
  ggplot(aes(x=interviewer_rating_1,color=gender)) +
  geom_boxplot()+
  theme_minimal()+
...
model_2 <-glm(as.factor(result)~gpa+as.factor(gender)+extracurriculars+work_experience
...
model_3 <-glm(as.factor(result)~gpa+as.factor(gender)+extracurriculars+work_experience
              +leadership_presence+speaking_skills+technical_skills+writing_skills + interviewer_rating_2,
              data=phase3,family=binomial("logit"))
summary(model_3)
tbl_regression(model_3, exponentiate = TRUE)
model_4<-glm(as.factor(result)~gpa+as.factor(gender)+extracurriculars+work_experience
              +leadership_presence+speaking_skills+technical_skills+writing_skills
...
```{r current_emp,include, echo=FALSE, message=FALSE}
...
ggsave("prod_plot.png", width = 7, height = 4)
