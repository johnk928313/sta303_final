...
In this part, we kept using the dataset in the first reasearch question. In order to investigate whether it is fair in the hiring process, we made barplots for final result by gender and also boxplot for gpa to check if final hired candidates had higher gpa than rejected ones.
...
As we can see in Figure 3, the boxplot for gpa of candidates grouped by gender and fianl hiring result, applicants who got hired have higher average of gpa than those who were rejected before. And there was no significant difference in gpa for those who got hired between gender. 

**\underline{Results}** \
  From the plot, we found that the final hired people did have higher gpa than the rejected ones. In order to check whethere the hiring process is fair, we can refer to the previous models we made for our first research problems. We found that in the result of model 1, gender is a significant variable while in model 2, gender is not a significant variable that had effects on the final hiring result. We might believe that no potential bias exists in the hiring phase 2 where we used AI service but there exist gender bias in the other phases.
...
**\underline{Data Wrangling and Visualization}** \
 ...
 We foun that in the dataset for the current employees, the salary is saved as character rather than number so we did some data wrangling and change other data type for future use.
```{r, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
...
![](images/productivity_salary.png)
 In Figure 5, current employees with higher productivity had lower productivity especially for those who prefer not to say their gender in most of teams which indicates potential bias existed in the salary process.
