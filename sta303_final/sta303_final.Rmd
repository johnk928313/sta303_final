---
title: "Gender bias in hiring and remuneration processes"
subtitle: "Subtitle"
...
phase1 %>% 
  select(team_applied_for,cover_letter,cv,gpa,gender,extracurriculars,work_experience) %>%
...
# check for gender distribution for each team
all_phases %>%
  ggplot(aes(x=final_result,fill=gender)) +
  geom_bar(position="fill")+
  facet_wrap(~team_applied_for)+
  theme_minimal()+
  labs(title="Gender distribution on final hiring results",
       x="Final result",
       y="Percentage",
...
# check gpa, work experience effect
all_phases %>%
ggplot(aes(y = factor(final_result), 
           x = gpa, 
           color = gender)) +
  geom_boxplot() +
...
model_1 <-glm(as.factor(result)~gpa+gender+work_experience,
              data=phase1,family=binomial("logit"))
summary(model_1)
tbl_regression(model_1, exponentiate = TRUE)

phase2<-as.data.frame(unclass(phase2))
model_2 <-glm(as.factor(result)~gpa+gender+work_experience
...
model_3 <-glm(as.factor(result)~gpa+gender
              +leadership_presence+speaking_skills+technical_skills+writing_skills
